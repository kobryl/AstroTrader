class Player {
    ...
    moveTowardsDestination(clientDeltaTime) {
        const dx = this.destinationPoint.x - this.x;
        const dy = this.destinationPoint.y - this.y;
        const angle = Math.atan2(dy, dx);
        const x = Config.PLAYER_SPEED * Math.cos(angle) * clientDeltaTime;
        const y = Config.PLAYER_SPEED * Math.sin(angle) * clientDeltaTime;
